...
test("server-renders the FirstFold Studio homepage", async () => {
  ...
  assert.match(html, /Founders Launch/);
  assert.match(html, /Toward Their Next Million/);
  assert.match(html, /funding, revenue, or first customers/);
  assert.doesNotMatch(html, /Founders Raise/);
  assert.doesNotMatch(html, /HelpingFounders|RaiseNext/);
  ...
  assert.match(html, /Independent website references/);
  assert.match(html, /Patterns we can learn from, not copy\./);
  assert.match(html, /These examples help us discuss useful direction, hierarchy, and proof/);
  assert.doesNotMatch(html, /Ready to Go Live Website Library/);
  assert.doesNotMatch(html, /Launch-ready references, curated for founders\./);
  ...
  assert.match(html, /One offer model\. Three support levels/);
  assert.match(html, /The same website foundation sits under Pro, Plus, and Master/);
  assert.match(html, /First launch/);
  assert.match(html, /Use Pro when the first public version needs to get clear and live/);
  assert.match(html, /More story/);
  assert.match(html, /Use Plus when the offer needs more room to explain, prove, and compare/);
  assert.match(html, /Hands-on release/);
  assert.match(html, /Use Master when launch support matters before and after the site goes live/);
  assert.match(html, /Founder-led offers/);
  assert.match(html, /AI-assisted speed/);
  assert.match(html, /Room to grow/);
  assert.doesNotMatch(html, /Creator-led brands/);
  assert.doesNotMatch(html, /Enterprise GTM/);
  assert.doesNotMatch(html, /Productized services/);
  ...
  assert.match(html, /The sprint can define a usable brand system/);
...
test("renders distinct secondary routes and removes conflicting offer copy", async () => {
  ...
  assert.doesNotMatch(`${work}${services}${process}${pricing}${resources}${about}${contact}`, /Case studies|How it Works|AI-Native Websites|Creator Packs|Enterprise Packs|\$4\.8k|from \$1\.6k/);
