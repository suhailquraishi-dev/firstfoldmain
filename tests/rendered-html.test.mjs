...
test("server-renders the FirstFold Studio homepage", async () => {
  ...
  assert.match(html, /Their Next Million/);
  ...
  assert.match(html, /Built for the first version that has to work/);
  assert.match(html, /Pro, Plus, and Master change the room and support level/);
...
test("renders distinct secondary routes and removes conflicting offer copy", async () => {
  ...
  assert.match(services, /A launch-ready website that makes the offer easier to buy/);
  ...
  assert.match(pricing, /Guaranteed performance outcomes/);
  assert.doesNotMatch(pricing, /Unverified performance guarantees/);
  ...
  assert.match(about, /Truthful product context/);
  assert.match(about, /Responsive launch checks before release/);
  ...
  assert.match(contact, /Send the Brief/);
  assert.match(contact, /Opens a draft in your email app/);
